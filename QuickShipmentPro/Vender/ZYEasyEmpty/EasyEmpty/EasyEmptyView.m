...
- (void)layoutemptyViewSubviews
{
    ...
    if (!ISEMPTY_S(self.emptyItem.imageName)) {
        UIImage *defaultImageA = [UIImage imageNamed:self.emptyItem.imageName];
        CGSize imageSize = defaultImageA.size ;
    ...
    self.bgContentView.frame = CGRectMake((self.easyS_width-contentWidth)/2, (self.easyS_height-contentHeight)/3, contentWidth, contentHeight);
...
- (UILabel *)defaultSubTitleLabel
{
    if (nil == _defaultSubTitleLabel) {
        ...
        _defaultSubTitleLabel.backgroundColor = [UIColor clearColor];
